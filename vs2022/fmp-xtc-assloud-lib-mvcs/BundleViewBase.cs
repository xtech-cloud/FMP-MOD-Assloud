...
namespace XTC.FMP.MOD.Assloud.LIB.MVCS
{
    ...
    public class BundleViewBase : View
    {
        ...
        /// <param name="_err">错误</param>
        /// <param name="_dto">UuidResponse的数据传输对象</param>
        public virtual void RefreshProtoTidy(Error _err, UuidResponseDTO _dto, object? _context)
        {
            var bridge = getFacade()?.getUiBridge() as IBundleUiBridge; 
            if (!Error.IsOK(_err))
            {
                bridge?.Alert(string.Format("errcode_Tidy_{0}", _err.getCode()), _err.getMessage(), _context);
                return;
            }
            bridge?.RefreshTidy(_dto, _context);
        }
